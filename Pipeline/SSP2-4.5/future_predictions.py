"""
Generate SSP2-4.5 yield predictions using a 5-model GCM ensemble.
...
print(f"\n{'='*60}")
print(f"Generating ensemble predictions (SSP2-4.5)")
print(f"{'='*60}")

gcm_predictions = {}  # {model_name: array of predictions}
base_future_df = None  # province/year structure from first available model
...
all_preds = np.array(list(gcm_predictions.values()))  # (n_models, n_samples)
...
if has_bootstrap:
    ...
    # Run bootstrap on each GCM's data, then average across GCMs
    ...
    boot_ensemble /= n_models  # average across GCMs
...
# Store per-GCM predictions
for gcm, preds in gcm_predictions.items():
    future_df[f'yield_{gcm}'] = preds

# Combined uncertainty: outer envelope of model spread and bootstrap PI
if has_bootstrap:
