"""Generate static choropleth maps for SSP2-4.5 banana yield projections.
...
def create_yield_map(merged_data, region, output_dir):
    ...
    filename = "banana_yield_map_ssp245.png" if region == "philippines" else f"banana_yield_map_{region}_ssp245.png"
...
def create_pct_change_map(merged_data, region, output_dir):
    ...
    filename = "banana_yield_pct_change_ssp245.png" if region == "philippines" else f"banana_yield_pct_change_{region}_ssp245.png"
    plt.savefig(os.path.join(output_dir, filename), dpi=600, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    print("All SSP2-4.5 maps created successfully!")
